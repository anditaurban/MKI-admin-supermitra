...
const allMenus = {
  ...
  mitra: { icon: "🤝", label: "Mitra" },
  product: { icon: "📋", label: "Kelola Produk" },
  course: { icon: "📚", label: "Kelola Course" },
  content: { icon: "📝", label: "Kelola Konten" },
};

const roleMenus = {
  superadmin: Object.keys(allMenus),
  owner: Object.keys(allMenus),
  finance: Object.keys(allMenus),
  sales: Object.keys(allMenus),
...
function toggleSubMenu(submenuId, iconId) {
  ...
  if (icon) {
    icon.classList.toggle("rotate-180"); // Memutar panah saat diklik
  }
}
...
function renderSidebar() {
  ...
  menuContainer.innerHTML = `
    ...
    <div class="flex flex-col space-y-1">
      <div class="flex items-center gap-2 py-2 px-2 hover:bg-blue-200 rounded-lg cursor-pointer transition-colors" onclick="toggleSubMenu('userSubmenu', 'userSubmenuIcon')">
        <span>👥</span>
      ...
      <div id="userSubmenu" class="hidden ml-8 flex flex-col space-y-1 border-l-2 border-gray-300 pl-2 mt-1">
...
// Panggil fungsi untuk merender (pastikan dipanggil setelah DOM siap)
renderSidebar();
